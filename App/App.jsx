...
import { AVATARS, CLASSES, FOOD, GAME_STATES, SYMBOLS } from '../constants';
...
function createGridRow (index, walls, [playerX, playerY], [foodX, foodY]) {
  ...
  // add the walls
  walls.forEach(([x, y]) => {
    if (y === index) row[x] = SYMBOLS.WALL;
  });

  // add the food
  if (foodY === index) row[foodX] = SYMBOLS.FOOD;

  // add the player
  if (playerY === index) row[playerX] = SYMBOLS.PLAYER;
...
function getAvatarForSymbol (symbol, currentFood, isGameOver) {
  ...
  if (isGameOver && symbol === SYMBOLS.PLAYER) return AVATARS.DEATH;
  // The food avatar is not defined in AVATARS, so fallback to that
...
function App () {
  ...
  const [vector, setVector] = useState(INITS.VECTOR);
  ...
  useInterval(() => {
    const nextPlayerXY = getNextPlayerXY(playerXY, vector);
    // Player has hit a roadblock
  ...
  return (
    <div>
      ...
      <div id="game" className={`is-${parseClassName(gameState)}`}>
        {grid.map((row, y) => {
          return (
            <div
              key={`row-${y}`}
            ...
            >
              {row.map((cellValue, x) => {
                return (
                  ...
                  >
                    <span>
                      {getAvatarForSymbol(cellValue, currentFood, gameState === GAME_STATES.GAME_OVER)}
                    </span>
                  </span>
                );
              })}
      ...
      <div id="info">
        {foodKeys.map(foodKey => `${FOOD[foodKey].AVATAR} = ${FOOD[foodKey].SCORE} points`).join(' ')}
      </div>
      {
        gameState === GAME_STATES.STARTUP && <Modal
          title="Welcome to the OAT Arcade!"
          confirmText="Start Driving"
          onConfirm={() => setGameState(GAME_STATES.RUNNING)}
        >
          <p>
            Help the OAT delivery driver to collect all the menu items
          </p>
          <p>
            Avoid the roadblocks, we don't want to keep our customers waiting...
          </p>
        </Modal>
      }
      {
        gameState === GAME_STATES.GAME_OVER && <Modal
          title="You Hit a Roadblock. Game Over!"
          confirmText="Play Again"
          onConfirm={() => resetGame()}
        >
          <p>
            You earned {score} points on your delivery
          </p>
        </Modal>
      }
    </div>
